...
class Employee(models.Model):
    ...
    _sql_constraints = [
        (
            "roll_number_unique",
            "unique(roll_number)",
            "The roll number is unique for each student.!",
        ),
        (
            "roll_number_check",
            "check(checker > 0 )",
            "The roll number is greater than zero!",
        ),
    ]
    ...
    job_history_ids = fields.One2many(
        "employee.job.history", "employee_id", string="Job History"
    )
    ...
    def create_sample_employee(self):
        self.env["employee.management"].create(
            {"name": "jmt", "roll_number": 84, "email": "jm123@gmail.com"}
        )
    ...
    def write_employee(self):
        employee = (
            self.env["employee.management"].browse(186).write({"job_title": "odoo"})
        )
    ...
    def search_count_employee(self):
        count = self.env["employee.management"].search_count(
            [("job_title", "=", "odoo")]
        )
        print("count..", count)
...
class WizardMarkDone(models.TransientModel):
    ...
    employee_id = fields.Many2one(
        "employee.management", string="Employee", required=True
    )
    note = fields.Text(string="Note")
    ...
    def default_get(self, default_fields):
        ...
        if employee_id:
            ...
            res.update({"employee_id": employee.id, "note": employee.note})
        return res

    def mark_done(self):
        self.ensure_one()
        existing = self.env["employee.management"].search(
            [("id", "=", self.employee_id.id), ("note", "=", self.note)]
        )
        if not existing:
